...
const validator = {
  ...
  nameInputBlurHandler(name, setError) {
    if (name === '') {
      ...
      setError('This field should have atleast 3 charecters.')
  ...
  nameInputChangeHandler(name, setError) {
    if (name.length === 0) {
      ...
      setError('should have atleast 3 characters.')
    } else if (name.slice(-1) === ' ') {
      setError('should not end with space.')
    } else {
      setError('')
  ...
  last_nameInputBlurHandler(name, setError) {
    if (name === '') {
      setError('This field cannot be empty!')
    } else if (name.length < 1) {
      setError('This field should have atleast 1 charecters.')
    } else if (name.slice(-1) === ' ') {
      setError('should not end with space.')
    } else {
      setError('')
  ...
  // @params  value, error state setter
  // @returns nothing
  last_nameInputChangeHandler(name, setError) {
    if (name.length === 0) {
      setError('This field cannot be empty!')
    } else if (name.charAt(0) === ' ') {
      setError('should not start with space.')
  ...
  customerIdInputBlurHandler(customerId, setError) {
    if (customerId === '') {
      setError('This field cannot be empty!')
    } else if (customerId.length < 3) {
      setError('Atleast contain 3 letters')
    }
    else {
      setError('')
    }
  ...
  // @params  percentage, error state setter
  // @returns nothing
  customerIdInputChangeHandler(customerId, setError) {
    if (customerId === '') {
      setError('This field cannot be empty!')
    } else {
      setError('')
